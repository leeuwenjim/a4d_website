class NewsControllPagination {
    ...
    data_to_news_item(data) {
        ...
        return $('<div>', {'class': 'content-container'}).append(
            $('<button>', {'id': 'delete_button', 'class': 'delete_button'}).css('float', 'right').text('Verwijder').on('click', function () {delete_news_item(data.id)}),
            $('<button>', {'id': 'edit_button', 'class': 'edit_button'}).css('float', 'right').text('Bewerk').on('click', () => {edit_item(data.id)}),
